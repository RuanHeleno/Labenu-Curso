...
app.post("/bands", async (req: Request, res: Response) => {
  try {
    ...
    res.send({ message: "Banda cadastrada com sucesso!" });
  } catch (error) {
    console.log(error);
...
app.post("/songs", async (req: Request, res: Response) => {
  try {
    ...
    res.send({ message: "Música registrada com sucesso!" });
  } catch (error) {
    console.log(error);
    ...
    try {
      ...
      if (newBandId !== undefined) {
        if (typeof newBandId !== "string") {
          res.status(400);
          throw new Error("'band_id' deve ser string");
        }
  
        if (newBandId.length !== 4) {
          res.status(400);
          throw new Error("'band_id' deve possuir 4 caracteres");
        }
      }
